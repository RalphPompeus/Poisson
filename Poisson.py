...
distribution = input("please choose type: Central or Wire:")
method = input("please choose algorithm: Jacobi, Gauss or GaussRelaxation?:")
...
def psi_update(psi_temp, rholattice,i,j,k):
    psi_new = (1/6)*(psi_temp[i+1,j,k] + psi_temp[i-1,j,k]+psi_temp[i,j+1,k] 
    + psi_temp[i,j-1,k]+psi_temp[i,j,k+1] + psi_temp[i,j,k-1]+ rholattice[i,j,k])
    return psi_new


def psi_Gauss(psi_temp, psi_current, rholattice, i, j, k, omega):
    psi_new = (omega/6)*(psi_current[i-1, j, k] + psi_temp[i+1, j, k]+ psi_current[i, j-1, k] 
    + psi_temp[i, j+1, k]+psi_current[i, j, k-1]+psi_temp[i, j, k+1]+rholattice[i,j,k])+(1-omega)*psi_current[i,j,k]
    return psi_new
...
def ELattice_update(psilattice, rholattice,dx,i,j,k):
    ...
    ####### do the ith, jth and kth terms need to be squared below!!!!!!!!!!!!!!
...
if (method == 'GAUSSRELAXATION'):
    ...
    for f in range(len(omegaRange)):
        ...
        while ((psierror_total > errorlimit)): 
            ...
            #print(t)
            toter = 0
            psi_temporary = np.copy(w)
            for i in range(1, (n-1)):
                for j in range(1,(n-1)):
                    for k in range(1,(n-1)):
                        w[i,j,k] = psi_Gauss(psi_temporary,w, rholattice,i,j,k, omegaRange[f])
                        #print(w)
                        normFactor = float(n**3)
                        psierror = abs(psilattice[i,j,k]-psi_temporary[i,j,k])
                        psierror = psierror/normFactor
                        toter += psierror   
                        print(toter)    
            #have tabbed below:            
            psierror_total = toter 
            totalPsiErrors.append(psierror_total)
            #print(psierror_total)
            #if (t == numsteps-1):
                #print("Error did not reach anticipated value")
            t_values += 1
            #t += 1
            print(t_values)
        iterationNumber.append(t_values)
        #print(iterationVals)
    plt.plot(omegaRange, iterationNumber)
    plt.show()
